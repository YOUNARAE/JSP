...
//	@RequestMapping("/member/memberInsert.do")
//	@GetMapping("/member/memberInsert.do") // 정확하게 겟메서드를 받겠다를 표현함
	@GetMapping // 동일 요청의 겟 메서드만 처리함
//	@Override
...
//	@RequestMapping(value="/member/memberInsert.do", method=RequestMethod.POST)
	...
	public String memberInsert(
			...
			, Errors errors //errors에 대한 모든 결과를 갖고 있음, 맵이 아닐 뿐이다.
//			, @RequestPart(value="memImage", required=false) MultipartFile memImage //이미 멤버 VO에 들어가있음
...
//			member.setMemImage(memImage); //이미지로 올라오는 녀석을 받아서 걔를 img로 바꿔주는데
			//setMemImage도 MemberVO에서 이미 다 set으로 처리해준 상태이기때문에 안 적어줘도 된다
//		}
...
//		Map<String, List<String>> errors = new LinkedHashMap<>();
		//맵을 미리 만들어놓고 
//		req.setAttribute("errors", errors); //아규먼트에 Errors errors의 코드로 모든 게 다 완료
		//미리 공유해놓기
