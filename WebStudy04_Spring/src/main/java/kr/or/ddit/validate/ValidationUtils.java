...
public class ValidationUtils {
	...
	public static <T> boolean validate(T target, Map<String, List<String>> erros, Class<?>...groups) { 
		//특정타입에 종속되지 않게 하려면 제네릭 타입으로 선언해줘야한다. MemberVO에만 쓸 수 있지 않도록, 검증 메소드니까 불린을 반환받는다
		...
		if(!valid) { //error니까 false일 때 작동한다
			constraintViolations.stream()
								.forEach(single->{
									String propertyName = single.getPropertyPath().toString(); 
									//우린 single로 키로 잡아야하기 때문에 String으로 해줘야해서 toString
